...
interface AnalysisResult {
  ...
  originalItem?: string;
  timestamp?: string;
  leadershipConstruct?: string;
  constructConfidence?: string;
  constructExplanation?: string;
}
...
const LeadershipMemoryTool = () => {
  ...
  const analyzeItem = async (useRuleBased = false) => {
    ...
    // Try API with enhanced analysis
    try {
          ...
          {showFallbackOption && (
            <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-lg">
              <p className="text-sm text-amber-800 mb-3">
                <strong>Note:</strong> Our alternative analysis method uses validated linguistic patterns to evaluate your items and provides reliable results.
              </p>
